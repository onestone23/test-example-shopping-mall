...
describe('debounce', () => {
  ...
  beforeEach(() => {
    ...
    // 타이머 모킹도 초기화 필수!
    // 써드파티 라이브러리, 전역의 teardown에서 타이머에 의존하는 로직 -> fakeTimer로 인해 제대로 동작하지 않을 수 있음
    vi.useFakeTimers();

    // 시간은 흐르기 때문에 매일 달라짐
    // -> 테스트 당시의 시간에 의존하는 테스트의 경우 시간을 고정하지 않으면 테스트가 깨질 수 있음
    // -> setSystemTime으로 시간을 고정하면 일관된 환경 테스트 가능
    vi.setSystemTiem(new Date('2023-12-25'));
  });
